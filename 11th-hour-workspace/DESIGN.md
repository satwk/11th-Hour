...
## 11th Hour Custom UI Interactions
While adhering strictly to the Notion design system above, implement these specific component interactions:
- **Plan Revision Card Stack:** Render as a vertical list. Use left border accents for AI actions: Teal (`border-teal-500`) = Task Reslotted, Amber (`border-amber-500`) = Urgency Downgraded, Coral (`border-rose-500`) = Draft ready. Always render the AI's reasoning text below the title in a muted text color.
- **Eisenhower Matrix:** You MUST use `@dnd-kit/core` and `@dnd-kit/sortable` for fluid drag-and-drop mechanics between the 4 quadrants. Do not use native HTML5 drag.
- **Focus Mode:** Use `framer-motion` for a smooth layout transition into a distraction-free, single-task view when "Start Focus Timer" is clicked.